...
function checkAndConvert(domain) {
  if (typeof domain === 'string') {
    debug('In checkAndConvert with string: '+domain);
    domain = new RegExp(domain.replace(/\./g, '\\.') + '$');
  } else if (!(domain instanceof RegExp)) {
    debug(inspect(domain));
    debug('In checkAndConvert with RegExp: '+domain.toString());
...
function redirect(options) {
  ...
  if (Object.prototype.toString.call(matchDomain) === '[object Array]') {
    debug('Received an array argument.');
  ...
  return function redirectMiddleware(req, res, next) {
    ...
    if (absolutePaths && info.pathname !== path.resolve(info.pathname)) {
      return next();
    }
